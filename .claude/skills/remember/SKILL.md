...
description: >
  ...
  defaults records to this agent's PERSONAL, PRIVATE scope (`--scope sensibo-cli
...
scope:** the default personal scope is **private** (`--scope sensibo-cli
...
- `--scope NAME` / `--visibility public|private` — record scope. **The wrapper
  ...
  (here, `sensibo-cli`). Private records are served only to a recall in the same
