...
type: command
description: >
  Cross-repo + mesh communication from sensibo-cli: file tracked GitHub issues
  ...
  the next step lives outside sensibo-cli (a brief for a sibling-repo agent, a
  status ping for a Culture channel, or pulling an issue body + comments
  into context). Issue posts auto-sign with `- sensibo-cli (Claude)`; mesh
  messages are unsigned (the IRC nick is the speaker). Not for in-sensibo-cli
